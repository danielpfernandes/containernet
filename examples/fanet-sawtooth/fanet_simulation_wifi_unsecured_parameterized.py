...
def simulate(number_of_drones: int = 5,
    ...
    # -------------------------------------- STAGE 1A -------------------------------------- #
    info(time_stamp() + "*** Stage 1A: BS1 sends initial coordinates to {}\n".format(drones[-1].name))
    ...
    # -------------------------------------- STAGE 2A -------------------------------------- #
    info(time_stamp() + "*** Stage 2A: BS1 changes the destination coordinates through {}\n".format(drones[1].name))
    info(time_stamp() + "*** Stage 2A Expected: Coordinates set to 50.02 10.02\n")
    set_rest_location(bs1, iterations=iterations_count, interval=wait_time_in_seconds,
                      target=drones[1].params['ip'], coordinates='50.02 10.02')
    # -------------------------------------- STAGE 3A -------------------------------------- #
    info(time_stamp() + "*** Stage 3A: {} is compromised and tries to change the destination coordinates\n".format(
        drones[-2].name))
